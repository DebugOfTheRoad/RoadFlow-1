UE.compule = {
    ...
    getRadioOrCheckboxHtml: function ($control, type)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            if ('radio' == type)
            {
                radios = '<span><%=BDictionary.GetRadiosByID("' + dictid + '".ToGuid(), "' + id + '", RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '")%>' + eventScripts + '</span>';
            }
            else if ('checkbox' == type)
            {
                radios = '<span><%=BDictionary.GetCheckboxsByID("' + dictid + '".ToGuid(), "' + id + '", RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '")%>' + eventScripts + '</span>';
            }
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            if ("radio" == type)
            {
                radios += '<span><%=new RoadFlow.Platform.WorkFlowForm().GetRadioFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '")%>' + eventScripts + '</span>';
            }
            else if ("checkbox" == type)
            {
                radios += '<span><%=new RoadFlow.Platform.WorkFlowForm().GetCheckboxFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '")%>' + eventScripts + '</span>';
            }
            $control.after(radios);
            $control.remove();
        }
    ...
    getSelectHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            radios += '<%=BDictionary.GetOptionsByID("' + dictid + '".ToGuid(), RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '")%>';
            radios += '</select>';
            var $radios = $(radios);
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            radios += '<%=new RoadFlow.Platform.WorkFlowForm().GetOptionsFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + defaultvalue + '")%>';
            radios += '</select>';
            var $radios = $(radios);
            $radios.attr("eventsid", $control.attr("eventsid"));
            $control.after($radios);
    ...
    getComboxHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            else
            {
                ...
                table += '>';
                table += '<%=BDictionary.GetComboxTableHtmlByID("' + dictid + '",RoadFlow.Platform.Dictionary.OptionValueField.ID,"' + defaultvalue + '")%>';
                table += '</span>';
                var $table = $(table);
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            else
            {
                ...
                table += '>';
                table += '<%=new RoadFlow.Platform.WorkFlowForm().GetComboxTableHtmlFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + defaultvalue + '")%>';
                table += '</span>';
                var $table = $(table);
                $table.attr("eventsid", $control.attr("eventsid"));
                $control.after($table);
        ...
        else if ("3" == datasource)//URL
        {
            if ("1" != listmode)
            {
                ...
                radios += '<%=new RoadFlow.Platform.WorkFlowForm().GetOptionsFromUrl("' + encodeURI(url) + '", "' + defaultvalue + '")%>';
                radios += '</select>';
                var $radios = $(radios);
                $radios.attr("eventsid", $control.attr("eventsid"));
                $control.after($radios);
    ...
    getDictHtml: function ($control)
    {
        ...
        switch (datasource)
        {
            ...
            case "1": //SQL
                ...
                $control.attr("class", "mylrselect").attr("title", "").attr("dbconn", ds_sql_dbconn).attr("sql", encodeURI(ds_sql_value));
            ...
            case "2": //URL
                ...
                $control.attr("class", "mylrselect").attr("title", "").attr("url0", encodeURI(ds_url_value0)).attr("url1", encodeURI(ds_url_value1)).attr("url2", encodeURI(ds_url_gettitle));
                $control.attr("isroot", isroot).attr("ismore", ismore).attr("isparent", isparent);
                $control.removeAttr("listtype").removeAttr("listtype").removeAttr("ds_dict_allchild");
                $control.removeAttr("ds_dict_value").removeAttr("ds_sql_dbconn").removeAttr("ds_sql_value").removeAttr("ds_url_value0");
                $control.removeAttr("ds_url_value1").removeAttr("ds_table_conn").removeAttr("ds_table_table").removeAttr("ds_table_valuefield");
            ...
            case "3": //表
                ...
                $control.attr("titlefield", ds_table_titlefield).attr("parentfield", ds_table_parentfield).attr("where", encodeURI(ds_table_where));
                $control.attr("isroot", isroot).attr("ismore", ismore).attr("isparent", isparent);
                $control.removeAttr("listtype").removeAttr("listtype").removeAttr("ds_dict_allchild");
                $control.removeAttr("ds_dict_value").removeAttr("ds_sql_dbconn").removeAttr("ds_sql_value").removeAttr("ds_url_value0");
                $control.removeAttr("ds_url_value1").removeAttr("ds_table_conn").removeAttr("ds_table_table").removeAttr("ds_table_valuefield");
    ...
    getHtmlHtml: function ($control, json)
    {
        ...
        //}
        textarea += '>';
        textarea += '<%=BWorkFlow.GetFromFieldData(initData, "' + table + '","' + field + '")%>';
        textarea += '</textarea>';
        $control.after(textarea);
        $control.remove();
    },
    ...
    getGridHtml: function ($control)
    {
        ...
        div += '" >';
        div += '<%=new RoadFlow.Platform.WorkFlowForm().GetFormGridHtml(DBConnID, "' + $control.attr("dataformat") + '","' + $control.attr("datasource") + '","' + $control.attr("datasource1") + '")%>';
        div += '</div>';
        $control.after(div);
        $control.remove();
    },
    ...
    getSubTableHtml_Select: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "select_dsdict":
                ...
                select += '<%=BDictionary.GetOptionsByID("' + rootid + '".ToGuid(), RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '")%>';
                break;
            case "select_dssql":
                ...
                select += '<%=new RoadFlow.Platform.WorkFlowForm().GetOptionsFromSql("' + conn + '", "' + sql.replaceAll('"', '\"') + '", "' + dvalue + '")%>';
                break;
    ...
    getSubTableHtml_Checkbox: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "checkbox_dsdict":
                ...
                checkbox = '<span><%=BDictionary.GetCheckboxsByID("' + rootid + '".ToGuid(), "' + name + '", RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'")%></span>';
                break;
            case "checkbox_dssql":
                var conn = editmode.checkbox_ds_dbconn;
                var sql = editmode.checkbox_ds_sql;
                checkbox = '<%=new RoadFlow.Platform.WorkFlowForm().GetCheckboxFromSql("' + conn + '", "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'")%>';
                break;
    ...
    getSubTableHtml_Radio: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "radio_dsdict":
                ...
                radio = '<span><%=BDictionary.GetRadiosByID("' + rootid + '".ToGuid(), "' + name + '", RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\' colname=\'' + colnumJSON.name + '\'")%></span>';
                break;
            case "radio_dssql":
                var conn = editmode.radio_ds_dbconn;
                var sql = editmode.radio_ds_sql;
                radio = '<%=new RoadFlow.Platform.WorkFlowForm().GetRadioFromSql("' + conn + '", "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\' colname=\'' + colnumJSON.name + '\'")%>';
                break;
            case "radio_dsstring":
    ...
    getSubTableHtml: function ($control)
    {
        ...
        {
            var index = 0;
            var html1 = '<table class="flowformsubtable" style="width:99%;margin:0 auto;" cellPadding="0" cellspacing="1" issubflowtable="1" id="subtable_' + id + '">';
            html1 += '<thead>';
            html1 += '<tr>';
            for (var i = 0; i < subtableJSON.colnums.length; i++)
            {
            ...
            html1 += '<input type="button" value="添加" onclick="subtable_addrow_' + id.replace('.', '') + '();"/>';
            html1 += '</th>'
            html1 += '</tr>';
            html1 += '</thead>';
            html1 += '<tbody>';
            ...
            for (var i = 0; i < subtableJSON.colnums.length; i++)
            {
                ...
                html1 += '<td>';
                html1 += '<%=bWorkFlowForm.GetDisplayString(dr["' + colnumJSON.fieldname + '"].ToString(), "' + (colnumJSON.displaymode || "normal") + '", "' + (colnumJSON.displaymodeformat || "") + '", DBConnID, "' + (colnumJSON.displaymodesql || "") + '" ) %>';
                html1 += '</td>';
            }
            html1 += '<td>';
            html1 += '<input type="button" value="编辑" onclick="subtable_editrow_' + id + '(\'<%=dr["' + subtableJSON.secondtableprimarykey + '"]%>\');" style="margin-right:4px;" name="flowsubtable_' + id + '_editbutton" />';
            html1 += '<input type="button" value="删除" onclick="subtable_deleterow_' + id + '(\'<%=dr["' + subtableJSON.secondtableprimarykey + '"]%>\');" name="flowsubtable_' + id + '_editbutton"/>';
            html1 += '</td>';
            html1 += '</tr>';
            html1 += '<%}%>';
            html1 += '</tbody>';
            ...
            script1 += 'new RoadUI.Window().open({url:"/Platform/WorkFlowRun/SubTableEdit.aspx?flowid=<%=FlowID%>&stepid=<%=StepID%>&secondtableeditform=' + subtableJSON.editform + '&primarytableid=" + instanceid + "&secondtablerelationfield=' + subtableJSON.secondtable + '.' + subtableJSON.secondtablerelationfield + '",openerid:tabid,title:"添加",height:' + (subtableJSON.displaymodeheight || 500) + ',width:' + (subtableJSON.displaymodewidth || 800) + '});';
            script1 += '}';
            script1 += 'function subtable_editrow_' + id + '(id)';
            script1 += '{';
            script1 += 'var subtableid="' + id + '";';
            script1 += 'var instanceid="<%=Request.QueryString["instanceid"]%>";';
            script1 += 'var tabid=\'<%=Request.QueryString["tabid"]%>\';';
            script1 += 'new RoadUI.Window().open({url:"/Platform/WorkFlowRun/SubTableEdit.aspx?flowid=<%=FlowID%>&stepid=<%=StepID%>&secondtableeditform=' + subtableJSON.editform + '&secondtable=' + subtableJSON.secondtable + '&secondtableconnid=<%=DBConnID%>&secondtableprimarykey=' + subtableJSON.secondtableprimarykey + '&instanceid="+id+"&primarytableid="+instanceid+"&secondtablerelationfield=' + subtableJSON.secondtable + '.' + subtableJSON.secondtablerelationfield + '",openerid:tabid,title:"编辑",height:' + (subtableJSON.displaymodeheight || 500) + ',width:' + (subtableJSON.displaymodewidth || 800) + '});';
            script1 += '}';
            ...
            script1 += '$.ajax({url:"/Platform/WorkFlowRun/SubTableDelete.aspx?secondtableeditform=' + subtableJSON.editform + '&secondtableconnid=<%=DBConnID%>&secondtable=' + subtableJSON.secondtable + '&secondtableprimarykey=' + subtableJSON.secondtableprimarykey + '&secondtablepkvalue="+id,async:false,cache:false,success:function(txt){alert(txt);window.location=window.location;}});';
            script1 += '}';
            script1 += '</script>';
